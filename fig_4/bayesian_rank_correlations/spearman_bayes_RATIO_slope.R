...
pre_mean_slope <- mining_data$pre_mean_slope[mining_data$per_Ross>0.9]
post_mean_slope <- mining_data$post_mean_slope[mining_data$per_Ross>0.9] 
y <- post_mean_slope / pre_mean_slope

rhoSamples_RATIO_slope <- spearmanGibbsSampler(xVals = x,
                                            yVals = y, 
                                            nSamples = 1e5,
                                            progBar = TRUE,
...
hist(rhoSamples_RATIO_slope$rhoSamples, freq = FALSE)

# Give the posterior samples for rho to the function below to compute BF01
spearman_bayes_factor_RATIO_slope <- computeBayesFactorOneZero(rhoSamples_RATIO_slope$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
...
cred_interval_99_RATIO_slope <- hdi(rhoSamples_RATIO_slope$rhoSamples, 0.99)
cred_interval_95_RATIO_slope <- hdi(rhoSamples_RATIO_slope$rhoSamples, 0.95)

d_RATIO_slope = density(rhoSamples_RATIO_slope$rhoSamples)
plot(d_RATIO_slope)

posterior_median <- median(rhoSamples_RATIO_slope$rhoSamples)
...
values_column <- c(posterior_median,
                   spearman_bayes_factor_RATIO_slope,
                   cred_interval_99_RATIO_slope[1],
                   cred_interval_99_RATIO_slope[2],
                   cred_interval_95_RATIO_slope[1],
                   cred_interval_95_RATIO_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, 'outputs/spearman_bayes_RATIO_slope.csv', row.names = FALSE)
write.csv(rhoSamples_RATIO_slope$rhoSamples, 'outputs/spearman_bayes_RATIO_slope_samples.csv', row.names = FALSE)