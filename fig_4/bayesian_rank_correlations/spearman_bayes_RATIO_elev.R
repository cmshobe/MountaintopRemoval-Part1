...
pre_mean_elev <- mining_data$pre_mean_elev[mining_data$per_Ross>0.9]
post_mean_elev <- mining_data$post_mean_elev[mining_data$per_Ross>0.9] 
y <- post_mean_elev / pre_mean_elev

rhoSamples_RATIO_elev <- spearmanGibbsSampler(xVals = x,
                                            yVals = y, 
                                            nSamples = 1e5,
                                            progBar = TRUE,
...
hist(rhoSamples_RATIO_elev$rhoSamples, freq = FALSE)

# Give the posterior samples for rho to the function below to compute BF01
spearman_bayes_factor_RATIO_elev <- computeBayesFactorOneZero(rhoSamples_RATIO_elev$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
...
cred_interval_99_RATIO_elev <- hdi(rhoSamples_RATIO_elev$rhoSamples, 0.99)
cred_interval_95_RATIO_elev <- hdi(rhoSamples_RATIO_elev$rhoSamples, 0.95)

d_RATIO_elev = density(rhoSamples_RATIO_elev$rhoSamples)
plot(d_RATIO_elev)

posterior_median <- median(rhoSamples_RATIO_elev$rhoSamples)
...
values_column <- c(posterior_median,
                   spearman_bayes_factor_RATIO_elev,
                   cred_interval_99_RATIO_elev[1],
                   cred_interval_99_RATIO_elev[2],
                   cred_interval_95_RATIO_elev[1],
                   cred_interval_95_RATIO_elev[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, 'outputs/spearman_bayes_RATIO_elev.csv', row.names = FALSE)
write.csv(rhoSamples_RATIO_elev$rhoSamples, 'outputs/spearman_bayes_RATIO_elev_samples.csv', row.names = FALSE)