...
y <- mining_data$W2_slope[mining_data$per_Ross>0.9] # elev wasserstein dist

rhoSamples_W2_slope <- spearmanGibbsSampler(xVals = x,
                                            yVals = y, 
                                            nSamples = 1e5,
                                            progBar = TRUE,
...
hist(rhoSamples_W2_slope$rhoSamples, freq = FALSE)

# Give the posterior samples for rho to the function below to compute BF01
spearman_bayes_factor_W2_slope <- computeBayesFactorOneZero(rhoSamples_W2_slope$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
...
cred_interval_99_W2_slope <- hdi(rhoSamples_W2_slope$rhoSamples, 0.99)
cred_interval_95_W2_slope <- hdi(rhoSamples_W2_slope$rhoSamples, 0.95)

d_W2_slope = density(rhoSamples_W2_slope$rhoSamples)
plot(d_W2_slope)

posterior_median <- median(rhoSamples_W2_slope$rhoSamples)
...
values_column <- c(posterior_median,
                   spearman_bayes_factor_W2_slope,
                   cred_interval_99_W2_slope[1],
                   cred_interval_99_W2_slope[2],
                   cred_interval_95_W2_slope[1],
                   cred_interval_95_W2_slope[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, 'outputs/spearman_bayes_W2_slope.csv', row.names = FALSE)
write.csv(rhoSamples_W2_slope$rhoSamples, 'outputs/spearman_bayes_W2_slope_samples.csv', row.names = FALSE)