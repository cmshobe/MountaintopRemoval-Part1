...
pre_mean_SA <- mining_data$pre_mean_SA[mining_data$per_Ross>0.9]
post_mean_SA <- mining_data$post_mean_SA[mining_data$per_Ross>0.9] 
y <- post_mean_SA / pre_mean_SA

rhoSamples_RATIO_SA <- spearmanGibbsSampler(xVals = x,
                                            yVals = y, 
                                            nSamples = 1e5,
                                            progBar = TRUE,
...
hist(rhoSamples_RATIO_SA$rhoSamples, freq = FALSE)

# Give the posterior samples for rho to the function below to compute BF01
spearman_bayes_factor_RATIO_SA <- computeBayesFactorOneZero(rhoSamples_RATIO_SA$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
...
cred_interval_99_RATIO_SA <- hdi(rhoSamples_RATIO_SA$rhoSamples, 0.99)
cred_interval_95_RATIO_SA <- hdi(rhoSamples_RATIO_SA$rhoSamples, 0.95)

d_RATIO_SA = density(rhoSamples_RATIO_SA$rhoSamples)
plot(d_RATIO_SA)

posterior_median <- median(rhoSamples_RATIO_SA$rhoSamples)
...
values_column <- c(posterior_median,
                   spearman_bayes_factor_RATIO_SA,
                   cred_interval_99_RATIO_SA[1],
                   cred_interval_99_RATIO_SA[2],
                   cred_interval_95_RATIO_SA[1],
                   cred_interval_95_RATIO_SA[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, 'outputs/spearman_bayes_RATIO_SA.csv', row.names = FALSE)
write.csv(rhoSamples_RATIO_SA$rhoSamples, 'outputs/spearman_bayes_RATIO_SA_samples.csv', row.names = FALSE)