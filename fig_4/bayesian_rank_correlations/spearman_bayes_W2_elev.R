...
y <- mining_data$W2_elev[mining_data$per_Ross>0.9] # elev wasserstein dist

rhoSamples_W2_elev <- spearmanGibbsSampler(xVals = x,
                                   yVals = y, 
                                   nSamples = 1e5,
                                   progBar = TRUE,
...
hist(rhoSamples_W2_elev$rhoSamples, freq = FALSE)

# Give the posterior samples for rho to the function below to compute BF01
spearman_bayes_factor_W2_elev <- computeBayesFactorOneZero(rhoSamples_W2_elev$rhoSamples, 
                          whichTest = "Spearman",
                          priorParameter = 1)
...
cred_interval_99_W2_elev <- hdi(rhoSamples_W2_elev$rhoSamples, 0.99)
cred_interval_95_W2_elev <- hdi(rhoSamples_W2_elev$rhoSamples, 0.95)

d_W2_elev <- density(rhoSamples_W2_elev$rhoSamples)
plot(d_W2_elev)

posterior_median <- median(rhoSamples_W2_elev$rhoSamples)
...
values_column <- c(posterior_median,
                   spearman_bayes_factor_W2_elev,
                   cred_interval_99_W2_elev[1],
                   cred_interval_99_W2_elev[2],
                   cred_interval_95_W2_elev[1],
                   cred_interval_95_W2_elev[2])

#build dataframe
export_df <- data.frame(labels_column, values_column)
write.csv(export_df, 'outputs/spearman_bayes_W2_elev.csv', row.names = FALSE)
write.csv(rhoSamples_W2_elev$rhoSamples, 'outputs/spearman_bayes_W2_elev_samples.csv', row.names = FALSE)